...
|---|---|---|---|
| `ICriticalSection` | `ICriticalSection_specification` | `[ICriticalSectionMatrix]` | Locked, TakeLock, reentrancy, timeouts, contention |
...
| `IInterprocessObject<T>` | `MachineWideSharedObjectTests` | `[XF]` | Persistence across dispose, data sharing (IAwaitableShared/IAwaitableProcessShared contracts covered by matrix specs above) |
...
- **`IInterprocessObject<T>`** has no dedicated matrix attribute. Its IAwaitableShared and IAwaitableProcessShared contracts are tested via `[IAwaitableSharedMatrix]` and `[IAwaitableProcessSharedMatrix]`, which both include InterprocessObject variants. `MachineWideSharedObjectTests` covers interprocess-object-specific concerns only.
