...
compares or narrates. Settled decisions are Magnus's (⚖); two things are deliberately left open and listed
at the end. Working name flagged inline: `LocalTessagingEngine` is not final. API sketches are
...
type. A tessage's type is its whole contract: its kind, its delivery guarantee, its transactionality, and
its remotability are all declared by the interfaces the type extends, and the machinery that carries a
tessage is chosen by reading its type.
...
- **TessageBus** — the asynchronous style: the message bus re-founded on tessages. Handlers publish
  **tevents** (multi-subscriber facts, routed by type compatibility) and send **tommands** (single-handler
  instructions, delivered exactly once, in order, per peer pair). Tell the world what happened — or direct
  one receiver — and trust it arrives.
...
handling, all of it synchronous, on the caller's thread, in the caller's execution context.
...
- **It has no runtime lifecycle.** Nothing about it starts, stops, or runs in the background; it is built
  with the container and is simply *there*.
...
One choreography executes any tessage's handler in its proper execution context:
...
- a **tevent** handler runs inside the publisher's transaction, as part of the publisher's unit of work.

Every arrival path calls this one executor: the local doors, an endpoint's inbox, an endpoint's transport
server. There is exactly one implementation of "run this handler correctly", and therefore exactly one
policy for its failures: a tessage of a single-handler kind that reaches an engine with no handler for it
raises the no-handler exception family — which, per
...
Every published tevent gets in-process participation — synchronous delivery to this engine's subscribed
handlers, inside the publisher's transaction. Whether a tevent *also* travels further is a property of its
type, honored by whatever delivery machinery the composition possesses; the publisher door is the same
either way.

### Tevent observation

Observation is the deliberately transaction-ignoring watch surface: an observer fires when a tevent
appears, outside any transaction, with no delivery guarantees — for read models of convenience, diagnostics,
and tests. ⚖ It is declared through the same builder as everything else, under its own verb, so the
distinct semantics are visible at the declaration site.
...
One handler registrar covers all four message kinds — the tessage's own type carries its kind and
guarantee, so the verbs differ only by handler shape:
...
container.Registrar.LocalTessagingEngine(engine => engine
   ...
   .RegisterTessageHandlers(handle => handle
      .ForTevent((IAccountTevent tevent, IUnitOfWorkResolver unitOfWork) => ...)
      .ForTuery((AccountTuery tuery, IScopeResolver scope) => ...)
      .ForTommand((ChangeEmail tommand, IUnitOfWorkResolver unitOfWork) => ...)
...
one transport server with one fresh-per-start address, discovery and announcement, the router, peer
memory, and the TessageBus delivery machinery of its tier. There are exactly two endpoint types, and they
differ **only in their TessageBus rung** — which delivery guarantees the endpoint can give:
...
- **The best-effort endpoint.** No database. Process-lifetime peer memory; per-peer best-effort tevent
  ...
  directly through the engine's executor.
- **The exactly-once endpoint.** Everything above, plus the durable vertical on a database foundation:
...
tessage's type. Typermedia behaves identically on both tiers — request/response neither queues nor
persists. A send whose guarantee the tier cannot honor fails loud: publishing an exactly-once tevent on a
best-effort endpoint is a composition error surfaced at the send, never a silent downgrade.
...
The router always maintains the connection to the endpoint's own inbox: a self-sent tommand rides the
outbox to the endpoint's own transport server like any other, so its delivery guarantees hold without a
special case. Tevents to self need no wire at all — they are the engine's in-process participation.
...
following the foundation: process-lifetime on the best-effort tier, database-backed on the exactly-once
tier. Remembered peers are what fan-out, receiver binding, queue-while-down, and waiting are computed
...
- **The transport protocol** — named pipes or ASP.NET Core; a strategy behind the one transport server and
  client.
- **The database foundation** — which engine backs the exactly-once tier's storage; typed so the pairing is
  routed by the compiler.
- **The serializer** — one parameter.
...
Topology is continuous convergence: addresses are ephemeral and never configured — an endpoint generates a
fresh address every start, announces it only while listening, and every router continuously reconciles its
connections against what the registry says now. Endpoints appearing, disappearing, and restarting at new
addresses are followed at signal latency. Identity is the `EndpointId`; the address is the instance.
...
**The host is a convenience, not infrastructure**: an optional object owning several endpoints' lifecycles
in one process, driving their phases together. Endpoints are first-class; a host adds nothing an endpoint
cannot do alone, and co-hosted endpoints converse exactly as separated ones do — real discovery, real
connections, real transport.
...
tracker's at-rest and rethrows background exceptions no assertion observed. The tracker is a testing
device; the production-honest await is quiescence.
