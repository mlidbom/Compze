...
class AspNetCoreTestServer : IAsyncDisposable
{
   readonly WebApplication _app;
   readonly HttpClient _client;

   AspNetCoreTestServer(WebApplication app, HttpClient client)
   {
      _app = app;
      _client = client;
   }

   public static async Task<AspNetCoreTestServer> StartAsync()
   {
      var compzeBuilder = DependencyInjectionContainerFactory.CreateContainerBuilder();
      ...
      DependencyInjectionContainerFactory.UseAsServiceProviderFor(compzeBuilder, builder.Host);
      ...
      return new AspNetCoreTestServer(app, client);
   }
...
public class When_a_compze_container_is_used_as_service_provider_for_AspNetCore
{
   public class singleton_services : When_a_compze_container_is_used_as_service_provider_for_AspNetCore
   {
      [DependencyInjectionContainerMatrix] public async Task returns_same_instance_across_multiple_requests()
      {
         await using var server = await AspNetCoreTestServer.StartAsync();
         var id1 = await server.GetStringAsync("/singleton");
         var id2 = await server.GetStringAsync("/singleton");
         id1.Must().Be(id2);
      }

      [DependencyInjectionContainerMatrix] public async Task returns_non_empty_value()
      {
         await using var server = await AspNetCoreTestServer.StartAsync();
   ...
   public class scoped_services : When_a_compze_container_is_used_as_service_provider_for_AspNetCore
   {
      [DependencyInjectionContainerMatrix] public async Task returns_different_instances_across_requests()
      {
         await using var server = await AspNetCoreTestServer.StartAsync();
      ...
      [DependencyInjectionContainerMatrix] public async Task returns_same_instance_within_a_single_request()
      {
         await using var server = await AspNetCoreTestServer.StartAsync();
   ...
   public class aspnetcore_framework_services : When_a_compze_container_is_used_as_service_provider_for_AspNetCore
   {
      [DependencyInjectionContainerMatrix] public async Task server_responds_to_http_requests()
      {
         await using var server = await AspNetCoreTestServer.StartAsync();
      ...
      [DependencyInjectionContainerMatrix] public async Task non_existent_endpoint_returns_404()
      {
         await using var server = await AspNetCoreTestServer.StartAsync();
