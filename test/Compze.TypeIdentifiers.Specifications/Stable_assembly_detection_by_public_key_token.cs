...
/// the assemblies that happened to be loaded when the mapper was constructed. So an assembly is recognised as
...
public class Stable_assembly_detection_by_public_key_token
{
   ...
   public class A_bare_mapper_with_nothing_registered : Stable_assembly_detection_by_public_key_token
   {
   ...
   {
      readonly TypeNameMapper _mapper = new();
      public After_trusting_the_assemblys_public_key_token() => _mapper.AddStablePublicKeyToken(CoreLibToken);
   ...
   {
      readonly TypeNameMapper _mapper = new();
      public After_declaring_the_assembly_stable_by_name() => _mapper.AddStableAssemblyName(typeof(string).Assembly.GetName().Name!);
