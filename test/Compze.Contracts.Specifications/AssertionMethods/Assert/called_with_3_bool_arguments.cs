...
public class called_with_3_bool_arguments : AssertionMethodsTest
{
   ...
   public class throws_if : called_with_3_bool_arguments
   {
      [XF] public void argument_1_is_false() =>
         Invoking(() => Asserter.Assert(false, true, true)).Must().Throw<AssertionTestException>();

      [XF] public void argument_2_is_false() =>
         Invoking(() => Asserter.Assert(true, false, true)).Must().Throw<AssertionTestException>();

      [XF] public void argument_3_is_false() =>
         Invoking(() => Asserter.Assert(true, true, false)).Must().Throw<AssertionTestException>();
   }

   public class exception_message_contains_the_argument_expression_if : called_with_3_bool_arguments
   {
