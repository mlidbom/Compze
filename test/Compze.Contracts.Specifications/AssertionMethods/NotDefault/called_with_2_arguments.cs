...
public class called_with_2_arguments : AssertionMethodsTest
{
   ...
   public class throws_with_message_containing_the_failing_argument_expression_if : called_with_2_arguments
   {
      ...
      [XF] public void argument_1_is_default() =>
         MustThrowContaining(() => Asserter.NotDefault(DefaultArg1, Guid.NewGuid()), nameof(DefaultArg1));

      [XF] public void argument_2_is_default() =>
         MustThrowContaining(() => Asserter.NotDefault(Guid.NewGuid(), DefaultArg2), nameof(DefaultArg2));
   }
}
