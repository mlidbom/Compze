      ...
      public partial class After_calling_AddEntity_with_name_RootEntity_and_a_new_Guid
      {
         public partial class Using_RootEntity
         {
            public partial class After_calling_AddEntity_with_name_entity1_and_a_newGuid : After_calling_AddEntity_with_name_RootEntity_and_a_new_Guid
            {
               ...
               [XF] public void Invoking_AddEntity_with_a_new_name_but_the_same_id_throws() => _rootEntity.Invoking(it => it.AddEntity("newEntityName", _entity1.Id)).Must().Throw<Exception>();
               [XF] public void QueryModel_has_a_single_entity_named_entity1() => _qmRootEntity.Entities.Single().Name.Must().Be("entity1");
               [XF] public void QueryModels_single_entity_has_the_same_Id_as_the_entity() => _qmRootEntity.Entities.Single().Id.Must().Be(_entity1.Id);

               public class The_Entities_collection : After_calling_AddEntity_with_name_entity1_and_a_newGuid
               {
                  [XF] public void Single_returns_the_entity() => _rootEntity.Entities.Single().Must().Be(_entity1);
                  [XF] public void InCreationOrder_Indexer_0_returns_the_entity() => _rootEntity.Entities.InCreationOrder[0].Must().Be(_entity1);
                  ...
                  {
                     [XF] public void Contains_returns_true() => _rootEntity.Entities.Contains(_entity1.Id).Must().Be(true);
                     [XF] public void Get_returns_the_entity() => _rootEntity.Entities.Get(_entity1.Id).Must().Be(_entity1);
                     [XF] public void Indexer_returns_the_entity() => _rootEntity.Entities[_entity1.Id].Must().Be(_entity1);

                     [XF] public void TryGet_returns_true_and_the_out_parameter_is_the_entity()
                     {
                        _rootEntity.Entities.TryGet(_entity1.Id, out var agEntity1Fetched).Must().BeTrue();
               ...
               {
                  [XF] public void Single_returns_the_entity_tuery_model() => _qmRootEntity.Entities.Single().Must().Be(_qmEntity1);
                  [XF] public void InCreationOrder_0_returns_the_entity_tuery_model() => _qmRootEntity.Entities.InCreationOrder[0].Must().Be(_qmEntity1);
                  ...
                  {
                     [XF] public void Contains_returns_true() => _qmRootEntity.Entities.Contains(_entity1.Id).Must().Be(true);
                     [XF] public void Get_returns_the_entity_tuery_model() => _qmRootEntity.Entities.Get(_entity1.Id).Must().Be(_qmEntity1);
                     [XF] public void Indexer_returns_the_entity() => _qmRootEntity.Entities[_entity1.Id].Must().Be(_qmEntity1);

                     [XF] public void TryGet_returns_true_and_the_out_parameter_is_the_entity()
                     {
                        _qmRootEntity.Entities.TryGet(_entity1.Id, out var qmEntity1Fetched).Must().BeTrue();
