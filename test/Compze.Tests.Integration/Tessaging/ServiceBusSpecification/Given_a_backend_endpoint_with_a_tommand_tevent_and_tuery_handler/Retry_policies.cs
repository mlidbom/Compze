...
public class Retry_policies_AtMostOnceTommand_when_tommand_handler_fails : EndpointHostTestBase
{
   protected override async Task InitializeAsyncInternal()
   {
      ...
      Host.AssertThatRunningScenarioThrowsBackendAndClientException<TransactionAbortedException>(action: () => Client.ExecuteRequest(navigator => navigator.Post(MyCreateTaggregateTommand.Create())));
