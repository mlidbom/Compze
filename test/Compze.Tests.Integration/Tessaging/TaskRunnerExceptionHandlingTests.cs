...
#pragma warning restore CA2213 // Disposable fields should be disposed
   ...
   public async Task Should_throw_taggregate_exception_on_dispose_when_background_task_throws()
   {
      await CompzeLogger.SuppressLoggingWhileRunningAsync(async () =>
      {
         ...
         _taskRunner.Run("test-task", () => gate.AwaitPassThrough()._tap(_ => throw new InvalidOperationException("exception1")));
   ...
   public async Task Should_collect_multiple_exceptions_from_multiple_background_tasks()
   {
      await CompzeLogger.SuppressLoggingWhileRunningAsync(async () =>
      {
         ...
         _taskRunner.Run("test-task-1", () => gate.AwaitPassThrough()._tap(_ => throw _exception1));
         _taskRunner.Run("test-task-2", () => gate.AwaitPassThrough()._tap(_ => throw _exception2));
         _taskRunner.Run("test-task-3", () => gate.AwaitPassThrough()._tap(_ => throw _exception3));
