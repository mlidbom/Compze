...
/// limit minus the longest generated identifier). Asserted loud at build, never sanitized silently.</summary>
public class Given_an_exactly_once_endpoint_declaration : UniversalTestBase
{
   [PCT] public async Task whose_name_contains_a_dot_building_fails_loud_naming_the_name_and_the_identifier_material_rule()
   {
   ...
   [PCT] public async Task whose_name_starts_with_a_digit_building_fails_loud_naming_the_identifier_material_rule()
   {
      await using var host = TestingEndpointHost.Create();
   ...
   [PCT] public async Task whose_name_exceeds_the_28_character_cap_building_fails_loud_naming_the_cap()
   {
      await using var host = TestingEndpointHost.Create();
   ...
   [PCT] public async Task whose_name_is_exactly_at_the_28_character_cap_builds()
   {
      await using var host = TestingEndpointHost.Create();
