...
public class MonitorCEPerformanceTests : UniversalTestBase
{
   ...
   class MyLong
   {
      ...
      {
         using(_awaitableLock.TakeReadLock())
         {
            return Read_Unsafe();
         }
      }

      internal long Read_MonitorCE_Read() => _awaitableLock.Read(Read_Unsafe);
      ...
      {
         using(_awaitableLock.TakeReadLock()) Increment_Unsafe();
      }

      internal void Increment_MonitorCE_Update() => _awaitableLock.Update(Increment_Unsafe);
   }
   ...
   [XF] public void _073_Increment_MonitorCE_Update_________________________time_is_less_than_nanoseconds_SingleThreaded_180__MultiThreaded_460() =>
      RunScenarios(() => _guarded.Increment_MonitorCE_Update(),
                   singleThreadAllowedTime: (180 * TotalLocks).Nanoseconds().EnvMultiply(instrumented: 45, unoptimized: 2.5),
