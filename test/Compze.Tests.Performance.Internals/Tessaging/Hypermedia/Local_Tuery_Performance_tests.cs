...
public class Local_Tuery_performance_tests : PerformanceTestBase
{
   [PCT]  public void Runs_10_000__MultiThreaded_local_requests_making_a_single_local_tuery_each_in_30_milliseconds() =>
      RunScenario(threaded: true, requests: 10_000.EnvDivide(instrumented:12), tueriesPerRequest: 1, maxTotal: TestEnv.DIContainer.ValueFor(autofac:60, microsoft:30, dryIoc:60).Milliseconds());

   [PCT]  public void Runs_10_000_SingleThreaded_local_requests_making_a_single_local_tuery_each_in_140_milliseconds() =>
      RunScenario(threaded: false, requests: 10_000.EnvDivide(instrumented:4), tueriesPerRequest: 1, maxTotal: TestEnv.DIContainer.ValueFor(autofac:280, microsoft:140, dryIoc:280).Milliseconds());

   [PCT]  public void Runs_4_000__MultiThreaded_local_requests_making_10_local_tueries_each_in_50_milliseconds() =>
      RunScenario(threaded: true, requests: 4_000.EnvDivide(instrumented:12), tueriesPerRequest: 10, maxTotal: TestEnv.DIContainer.ValueFor(autofac:100, microsoft:50, dryIoc:100).Milliseconds());

   [PCT]  public void Runs_10_000__SingleThreaded_local_requests_making_10_local_tueries_each_in_170_milliseconds() =>
      RunScenario(threaded: false, requests: 10_000.EnvDivide(instrumented:6), tueriesPerRequest: 10, maxTotal: TestEnv.DIContainer.ValueFor(autofac:300, microsoft:170, dryIoc:300).Milliseconds());
   ...
   void RunScenario(bool threaded, int requests, int tueriesPerRequest, TimeSpan maxTotal)
   {
      ...
      void RunRequest() =>
         ...
         {
            var navigator = scope.Resolve<IUnitOfWorkLocalTypermediaNavigator>();
