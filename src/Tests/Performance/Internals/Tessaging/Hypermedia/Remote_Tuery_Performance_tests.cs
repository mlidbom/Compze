...
using CreatesItsOwnResultTuery = Compze.Abstractions.Tessaging.Public.TessageTypes.Remotable.NonTransactional.Queries.NewableResultLink<Compze.Tests.Performance.Internals.Tessaging.Hypermedia.PerformanceTestBase.MyTueryResult>;
...
public class RemoteTueryPerformanceTests : PerformanceTestBase
{
   ...
   [PCT]  public void SingleThreaded_Runs_100_local_requests_making_one_ICreateMyOwnResult_tuery_each_in_2_milliseconds() =>
      RunScenario(threaded: false, requests: 100, queriesPerRequest: 1, maxTotal: 2.Milliseconds().EnvMultiply(instrumented:2.4), tuery: new CreatesItsOwnResultTuery());
   ...
   [PCT]  public void SingleThreaded_Runs_10_local_requests_making_10_ICreateMyOwnResult_tuery_each_in_2_milliseconds() =>
      RunScenario(threaded: false, requests: 10, queriesPerRequest: 10, maxTotal: 2.Milliseconds().EnvMultiply(instrumented:1.3), tuery: new CreatesItsOwnResultTuery());

   [PCT]  public void SingleThreaded_Runs_1_local_request_making_200_ICreateMyOwnResult_tuery_each_in_1_milliseconds() =>
      RunScenario(threaded: false, requests: 1, queriesPerRequest: 200, maxTotal: 1.Milliseconds().EnvMultiply(instrumented:2.6), tuery: new CreatesItsOwnResultTuery());
   ...
   [PCT]  public void MultiThreaded_Runs_10_local_request_making_200_ICreateMyOwnResult_tuery_each_in_15_milliseconds() =>
      RunScenario(threaded: true, requests: 10, queriesPerRequest: 200, maxTotal: 15.Milliseconds().EnvMultiply(instrumented:2.2), tuery: new CreatesItsOwnResultTuery());
   ...
   [PCT]  public async Task Async_Runs_10_local_request_making_200_ICreateMyOwnResult_tuery_each_in_15_milliseconds() =>
      await RunAsyncScenario(requests: 10, queriesPerRequest: 200, maxTotal: 15.Milliseconds().EnvMultiply(instrumented:3.4), tuery: new CreatesItsOwnResultTuery());


   void RunScenario(bool threaded, int requests, int queriesPerRequest, TimeSpan maxTotal, IRemotableTuery<MyTueryResult> tuery)
   {
   ...
   async Task RunAsyncScenario(int requests, int queriesPerRequest, TimeSpan maxTotal, IRemotableTuery<MyTueryResult> tuery)
   {
      var navigationSpecification = NavigationSpecification.Get(tuery);
