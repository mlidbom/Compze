...
#pragma warning restore CA1812 // Class is never instantiated
   ...
   [XF] public void Allocated_and_disposes_100_instances_in_100_millisecond_when_actually_collecting_stack_traces()
   {
      TimeAsserter.Execute(action: () => new StrictlyManagedResource<StrictResource>(forceStackTraceCollection: true).Dispose(),
                           iterations: 100,
                           maxTotal: 100.Milliseconds().EnvMultiply(unoptimized: 1.3));
   }
