...
[TestFixture] public class SqliteReadOrderTests : UniversalTestBase
{
   [Test] public void Parse_followed_by_ToString_always_results_in_identical_string()
   {
      ...
      ReadOrder.Parse($"{long.MaxValue}.{long.MaxValue}").ToSqliteReadOrderString().Should().Be(maxValue);
      ...
      ReadOrder.Parse(CreateString(1, 1)).ToSqliteReadOrderString().Should().Be(expectedPadded);
   }
   ...
   [Test] public void RoundTripping_SqlDecimal_results_in_same_value()
   {
      ...
      {
         var stringValue = value.ToSqliteReadOrderString();
         ...
         ReadOrder.FromSqlDecimal(sql).ToSqliteReadOrderString().Should().Be(stringValue);
      }
   }
