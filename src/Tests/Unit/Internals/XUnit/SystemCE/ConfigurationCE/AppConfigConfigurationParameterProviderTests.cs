...
public class AppConfigConfigurationParameterProviderTests: XUnitTestBase
{
   ...
   [XFact] public void ParameterProvider_should_return_the_value_specified_in_the_configuration_file() =>
      _provider.GetString("KeyTest1").Should().Be("ValueTest1");

   [XFact] public void ParameterProvider_should_throw_ConfigurationErrorsException_when_key_does_not_exist() =>
      this.Invoking(_ => _provider.GetString("ErrorTest1")).Should().Throw<Exception>();

   [XFact] public void ParameterProvider_exception_should_contain_parameter_key() =>
