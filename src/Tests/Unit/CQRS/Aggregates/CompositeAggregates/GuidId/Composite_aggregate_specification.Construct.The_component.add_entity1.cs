   ...
   public partial class After_constructing_root_aggregate_with_name_root_and_slaving_a_query_model_to_the_aggregates_events
   {
      public partial class The_component
      {
         public partial class After_calling_AddEntity_with_name_entity1_and_a_newGuid : The_component
         {
            ...
            [XF] public void Invoking_AddEntity_with_a_new_name_but_the_same_id_throws() => _component.Invoking(it => it.AddEntity("newEntityName", _entity1.Id)).Should().Throw<Exception>();
            [XF] public void QueryModel_has_a_single_entity_named_entity1() => _qmComponent.Entities.Single().Name.Should().Be("entity1");
            [XF] public void QueryModels_single_entity_has_the_same_Id_as_the_entity() => _qmComponent.Entities.Single().Id.Should().Be(_entity1.Id);

            public class The_Entities_collection : After_calling_AddEntity_with_name_entity1_and_a_newGuid
            {
               [XF] public void Single_returns_the_entity() => _component.Entities.Single().Should().Be(_entity1);
               [XF] public void InCreationOrder_0_returns_the_entity() => _component.Entities.InCreationOrder[0].Should().Be(_entity1);
               ...
               {
                  [XF] public void Contains_returns_true() => _component.Entities.Contains(_entity1.Id).Should().Be(true);
                  [XF] public void Get_returns_the_entity() => _component.Entities.Get(_entity1.Id).Should().Be(_entity1);
                  [XF] public void Indexer_returns_the_entity() => _component.Entities[_entity1.Id].Should().Be(_entity1);
