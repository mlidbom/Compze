...
public static partial class Composite_taggregate_specification
{
   public partial class After_constructing_root_taggregate_with_name_root_and_slaving_a_tuery_model_to_the_taggregates_tevents
   {
      public partial class After_adding_entity_named_entity1 : After_constructing_root_taggregate_with_name_root_and_slaving_a_tuery_model_to_the_taggregates_tevents
      {
         ...
         {
            [XF] public void Single_returns_the_entity() => _taggregate.Entities.Single().Should().Be(_entity1);
            [XF] public void InCreationOrder_0_returns_the_entity() => _taggregate.Entities.InCreationOrder[0].Should().Be(_entity1);
            ...
            {
               [XF] public void Contains_returns_true() => _taggregate.Entities.Contains(_entity1.Id).Should().Be(true);
               [XF] public void Get_returns_the_entity() => _taggregate.Entities.Get(_entity1.Id).Should().Be(_entity1);
               [XF] public void Indexer_returns_the_entity() => _taggregate.Entities[_entity1.Id].Should().Be(_entity1);

               [XF] public void TryGet_returns_true_and_the_out_parameter_is_the_entity()
               {
                  _taggregate.Entities.TryGet(_entity1.Id, out var agEntity1Fetched).Should().BeTrue();
