      ...
      public partial class The_component
      {
         public class After_calling_AddEntity_with_name_entity1_and_a_newGuid : The_component
         {
            ...
            public After_calling_AddEntity_with_name_entity1_and_a_newGuid()
            {
               ...
               _entity1 = _agComponent.AddEntity("entity1", _entity1Id);
            }
         }

         [XFact] public void Aggregate_Component_Entity_tests()
         {
            var _agComponent = Aggregate.Component;
            var _qmComponent = QueryModel.Component;

            var _entity1Id = Guid.NewGuid();
            var _entity1 = _agComponent.AddEntity("entity1", _entity1Id);
            _agComponent.Invoking(it => it.AddEntity("entity2", _entity1Id)).Should().Throw<Exception>();
            ...
            _agComponent.Entities.InCreationOrder.Count.Should().Be(1);
            _qmComponent.Entities.InCreationOrder.Count.Should().Be(1);
            _agComponent.Entities.Contains(_entity1.Id).Should().Be(true);
            _qmComponent.Entities.Contains(_entity1.Id).Should().Be(true);
            _agComponent.Entities.Get(_entity1.Id).Should().Be(_entity1);
            _qmComponent.Entities.Get(_entity1.Id).Should().Be(qmEntity1);
            _agComponent.Entities[_entity1.Id].Should().Be(_entity1);
            _qmComponent.Entities[_entity1.Id].Should().Be(qmEntity1);

            var entity2Id = Guid.NewGuid();
            var agEntity2 = _agComponent.AddEntity("entity2", entity2Id);
            _agComponent.Invoking(it => it.AddEntity("entity3", entity2Id)).Should().Throw<Exception>();

            var qmEntity2 = _qmComponent.Entities.InCreationOrder[1];
            agEntity2.Name.Should().Be("entity2");
            qmEntity2.Name.Should().Be("entity2");
            _agComponent.Entities.InCreationOrder.Count.Should().Be(2);
            _qmComponent.Entities.InCreationOrder.Count.Should().Be(2);
            _agComponent.Entities.Contains(agEntity2.Id).Should().Be(true);
            _qmComponent.Entities.Contains(agEntity2.Id).Should().Be(true);
            _agComponent.Entities[agEntity2.Id].Should().Be(agEntity2);
            ...
            agEntity2.Remove();
            _agComponent.Entities.Contains(agEntity2.Id).Should().Be(false);
            _qmComponent.Entities.Contains(agEntity2.Id).Should().Be(false);
            _agComponent.Entities.InCreationOrder.Count.Should().Be(1);
            _qmComponent.Entities.InCreationOrder.Count.Should().Be(1);
            _agComponent.Invoking(it => it.Entities.Get(agEntity2.Id)).Should().Throw<Exception>();
            _qmComponent.Invoking(it => it.Entities.Get(agEntity2.Id)).Should().Throw<Exception>();
            _agComponent.Invoking(it => { _ = it.Entities[agEntity2.Id]; }).Should().Throw<Exception>();
            _qmComponent.Invoking(it => { _ = it.Entities[agEntity2.Id]; }).Should().Throw<Exception>();

            _entity1.Remove();
            _agComponent.Entities.Contains(_entity1.Id).Should().Be(false);
            _qmComponent.Entities.Contains(_entity1.Id).Should().Be(false);
            _agComponent.Entities.InCreationOrder.Count.Should().Be(0);
            _qmComponent.Entities.InCreationOrder.Count.Should().Be(0);
            _agComponent.Invoking(it => it.Entities.Get(_entity1.Id)).Should().Throw<Exception>();
            _qmComponent.Invoking(it => it.Entities.Get(_entity1.Id)).Should().Throw<Exception>();
            _agComponent.Invoking(it => { _ = it.Entities[_entity1.Id]; }).Should().Throw<Exception>();
