...
/// The independent counterpart of <see cref="IUnitOfWorkLocalTypermediaNavigator"/>, which navigates within the caller's<br/>
...
/// <see cref="IUnitOfWorkLocalTypermediaNavigator"/>, which deliberately joins it. Resolvable from the root: a singleton, so<br/>
...
public interface IIndependentLocalTypermediaNavigator
{
   ...
   /// see <see cref="IUnitOfWorkLocalTypermediaNavigator.Execute{TTuery,TResult}"/>.</summary>
   ...
   /// returns — see <see cref="IUnitOfWorkLocalTypermediaNavigator.Execute{TResult}(IStrictlyLocalTommand{TResult})"/>.</summary>
   TResult Execute<TResult>(IStrictlyLocalTommand<TResult> tommand);

   ///<summary>Executes the local handler for <paramref name="tommand"/> as its own unit of work, committed when the call<br/>
   /// returns — see <see cref="IUnitOfWorkLocalTypermediaNavigator.Execute(IStrictlyLocalTommand)"/>.</summary>
