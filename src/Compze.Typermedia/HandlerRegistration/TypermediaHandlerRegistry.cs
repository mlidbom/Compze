...
public sealed class TypermediaHandlerRegistry(ITypeMapper typeMapper) : ITypermediaHandlerRegistrar, ITypermediaHandlerRegistry
{
   ...
   IReadOnlyDictionary<Type, Action<object, IScopeServiceLocator>> _voidTommandHandlers = new Dictionary<Type, Action<object, IScopeServiceLocator>>();

   readonly IMonitor _monitor = IMonitor.New();

   ITypermediaHandlerRegistrar ITypermediaHandlerRegistrar.ForTommand<TTommand>(Action<TTommand, IScopeServiceLocator> handler) => _monitor.Locked(() =>
   {
   ...
   ITypermediaHandlerRegistrar ITypermediaHandlerRegistrar.ForTommand<TTommand, TResult>(Func<TTommand, IScopeServiceLocator, TResult> handler) => _monitor.Locked(() =>
   {
      TessageTypeInspector.AssertValid(typeof(TTommand));
   ...
   ITypermediaHandlerRegistrar ITypermediaHandlerRegistrar.ForTuery<TTuery, TResult>(Func<TTuery, IScopeServiceLocator, TResult> handler) => _monitor.Locked(() =>
   {
   ...
   public Action<ITommand, IScopeServiceLocator> GetVoidTommandHandler(ITommand tommand)
   {
   ...
   public Func<ITommand, IScopeServiceLocator, object> GetTommandHandlerWithReturnValue(Type tommandType) => _tommandHandlersReturningResults[tommandType].HandlerMethod;

   public Func<ITuery<object>, IScopeServiceLocator, object> GetTueryHandler(Type tueryType) => _tueryHandlers[tueryType].HandlerMethod;

   public Func<IStrictlyLocalTuery<TTuery, TResult>, IScopeServiceLocator, TResult> GetTueryHandler<TTuery, TResult>(IStrictlyLocalTuery<TTuery, TResult> tuery) where TTuery : IStrictlyLocalTuery<TTuery, TResult>
   {
   ...
   public Func<ITommand<TResult>, IScopeServiceLocator, TResult> GetTommandHandler<TResult>(ITommand<TResult> tommand)
   {
   ...
   abstract class HandlerWithResultRegistration(Type returnValueType, Func<object, IScopeServiceLocator, object> handlerMethod)
   {
      internal Type ReturnValueType { get; } = returnValueType;
      internal Func<object, IScopeServiceLocator, object> HandlerMethod { get; } = handlerMethod;
   }

   class TommandHandlerWithResultRegistration<TTommand, TResult>(Func<TTommand, IScopeServiceLocator, TResult> handlerMethod) : HandlerWithResultRegistration(typeof(TResult),
                                                                                                                                                               (tommand, kernel) => handlerMethod((TTommand)tommand, kernel)!);

   class TueryHandlerRegistration<TTuery, TResult>(Func<TTuery, IScopeServiceLocator, TResult> handlerMethod) : HandlerWithResultRegistration(typeof(TResult),
