...
public static class EndpointBuilderTypermediaExtensions
{
   extension(IEndpointBuilder @this)
   {
      ...
      /// strictly local tueries and tommands execute synchronously, on the calling thread, within the
      /// caller's transaction, through the <see cref="IUnitOfWorkLocalTypermediaNavigator"/>. Wires no transport
      /// server and no discovery.<br/>
