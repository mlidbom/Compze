...
- `NewtonsoftSerializer()`: fills the one serializer parameter an endpoint's declaration surface takes (`EndpointBuilder.Serializer` — registering both wire serializers in one declaration) and the pure client's (`TypermediaClientBuilder.Serializer`).
