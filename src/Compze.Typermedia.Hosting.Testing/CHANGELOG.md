...
- `DistributedTypermediaTestingEndpointHostFeature` declares `DiscoverEndpointsThrough` an `IEndpointRegistry` listing the host's typermedia addresses, so every endpoint's typermedia router connects to every endpoint in the host — endpoints in a testing host navigate each other's typermedia exactly as they would across processes.
