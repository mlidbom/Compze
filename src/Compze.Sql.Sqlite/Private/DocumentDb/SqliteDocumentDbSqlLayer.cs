...
public partial class SqliteDocumentDbSqlLayer : IDocumentDbSqlLayer
{
   ...
   static string TypeInClause(IReadOnlySet<TypeId> acceptableTypeIds) => ContractAssertion.Argument.Fulfills(acceptableTypeIds.Any())._then("IN( '" + acceptableTypeIds.Select(guid => guid.ToString()).Join("', '") + "')\n");
