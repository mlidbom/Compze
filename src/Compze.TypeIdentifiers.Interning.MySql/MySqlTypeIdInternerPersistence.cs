...
namespace Compze.TypeIdentifiers.Interning.MySql;

partial class MySqlTypeIdInternerPersistence(IMySqlConnectionPool connectionPool, MySqlSqlLayerSchemaManager schemaManager) : ITypeIdInternerPersistence
{
   ...
   // MySQL/InnoDB is MVCC: a suppressed mint commits independently of the business transaction.
   ...
   public T MutateUnderWriteLock<T>(Func<IInternerWriteSession, T> work) => TransactionScopeCe.Execute(() =>
      _connectionPool.UseConnection(connection =>
      {
         connection.UseCommand(command =>
            command.SetCommandText("SELECT GET_LOCK(@resource, -1)")
                   .AddVarcharParameter("resource", 64, LockResource)
                   .ExecuteNonQuery());
         try
         {
         ...
         {
            connection.UseCommand(command =>
               command.SetCommandText("SELECT RELEASE_LOCK(@resource)")
                      .AddVarcharParameter("resource", 64, LockResource)
                      .ExecuteNonQuery());
         }
      }), TransactionScopeOption.Suppress);

   sealed class WriteSession(ICompzeMySqlConnection connection) : IInternerWriteSession
   {
      readonly ICompzeMySqlConnection _connection = connection;
      ...
      public int InsertType(string fullyQualifiedName, string spelling)
      {
         ...
         var id = _connection.UseCommand(command =>
            NonNullInt(command.SetCommandText("SELECT LAST_INSERT_ID()").ExecuteScalar()));
