...
- The unit of work is a first-class concept (see `dev_docs/unit-of-work-model.md`): `ExecuteUnitOfWork` (formerly `ExecuteTransactionInIsolatedScope` — that name described the mechanics; this one names the concept: one scope and one transaction, begun and completed together, committing or rolling back as a whole) hands its callback an `IUnitOfWorkResolver : IScopeResolver` — the typed statement that code runs inside a unit of work. The container never grants the typing (a scope is not necessarily a unit of work); only `ExecuteUnitOfWork` and `UnitOfWorkResolver.From`, which asserts the ambient transaction, do. `ExecuteInIsolatedScope` stays as the deliberately-not-a-unit-of-work context for work that changes nothing. `ExecuteInIsolatedScopeAsync` deleted: zero callers.
- **`IUnitOfWorkResolver` is the unit of work's handle, not only its resolver.** `Id` — a `UnitOfWorkId`, value-equal exactly when it identifies the same unit of work (backed by the transaction's `LocalIdentifier`) — keys logs, caches, and idempotence checks on "which unit of work did this". `OnCommittedSuccessfully(action)` runs after the unit of work commits and never on rollback (the send-on-commit pattern); `OnCompleted(action)` runs however it ends (the cleanup pattern) — the same `TransactionCompleted`-based callbacks the outbox and the connection cleanup already trusted, surfaced on the unit of work's own handle. All three operate on the transaction captured when the resolver was created, never on `Transaction.Current`, so they bind to the unit of work the resolver certifies even where the ambient transaction has drifted.
- **`Lifestyle.UnitOfWork` — `UnitOfWork.For<TService>()`.** A `Scoped` component that additionally requires an ambient transaction: it participates in the enclosing unit of work, and resolving it with no ambient transaction present throws, naming the missing transaction. The backend containers never see the lifestyle — they are handed a `Scoped` registration whose instantiation asserts the ambient transaction first — so no backend adapter knows units of work exist. Deliberately single-service (no multi-service `For` overloads): one instance serving several service types usually pairs an updating face with a reading face, and a reading face must construct in plain read scopes. A singleton depending on such a component fails `Build()` exactly as it would for `Scoped`. `WithServiceResolver()` on a `UnitOfWork` registration registers the deferred resolver as `Scoped`: the resolver is the deferral seam, so the transaction requirement bites at `Resolve()` — when the target instantiates — never when the resolver itself is constructed. The assert fires at instantiation (the first resolution in each scope), proving the component is born inside a unit of work — not that it never outlives its transaction: component-level usage guards remain the deep defense.

### Added

- **`IComponentSet<TService>` — component sets as ordinary constructor dependencies.** The injectable counterpart of `ResolveSet`: a component takes `IComponentSet<TService>` through `CreatedBy(...)` and receives the whole set. The container synthesizes the one `IComponentSet<TService>` registration per set service type at `Build()`; its lifestyle follows the members' (Singleton when every member is, Scoped otherwise), so lifestyle validation guards a set dependency exactly like a member dependency; members resolve on enumeration, staying transparent to scope — the `IServiceResolver<TService>` thin-view philosophy. Hand-written `IComponentSet` registrations are rejected (the type is container vocabulary; clone and child containers pass because their registrations carry the container's own `ComponentSet<TService>` implementation), and the raw-member-dependency `Build()` error now recommends `IComponentSet<T>`. A set nothing contributed to is still a set — the empty one: a `CreatedBy(...)` dependency on an `IComponentSet<TService>` with no `ForSet(...)` members receives the empty set (synthesized as a singleton), because zero contributions is a legitimate state for a contribution seam, not a wiring error.
...
- **Component sets now cross into child containers.** `CreateChildContainerBuilder()` delegated every singleton to the parent by resolving it singularly — forbidden (and ambiguous) for a component set's service type, so any set member broke child-container creation. Singleton set members now delegate like every other singleton — same instances, not disposed by the child — fetched through the parent's `ResolveSet`; non-singleton members are copied like their singular counterparts. A set mixing singleton and non-singleton members is rejected with a clear message, since fetching its singleton instances would force the non-singleton members to instantiate from the parent's root scope.
