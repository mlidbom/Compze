...
- **The async unit-of-work envelope**: `ExecuteUnitOfWorkAsync` (action and result forms) pairs one fresh scope with one transaction whose ambient flows across the function's awaits — one unit of work legitimately migrates across pool threads, its identity its transaction — and `ExecuteInIsolatedScopeAsync` (both forms) is its no-transaction sibling for reads. See `dev_docs/unit-of-work.md`, which now records the async-flow-safety contract and the session-affinity consequence: session usage guards assert transaction affinity, never thread affinity.
- The unit of work is a first-class concept (see `dev_docs/unit-of-work.md`): `ExecuteUnitOfWork` (formerly `ExecuteTransactionInIsolatedScope` — that name described the mechanics; this one names the concept: one scope and one transaction, begun and completed together, committing or rolling back as a whole) hands its callback an `IUnitOfWorkResolver : IScopeResolver` — the typed statement that code runs inside a unit of work. The container never grants the typing (a scope is not necessarily a unit of work); only `ExecuteUnitOfWork` and `UnitOfWorkResolver.From`, which asserts the ambient transaction, do. `ExecuteInIsolatedScope` stays as the deliberately-not-a-unit-of-work context for work that changes nothing. `ExecuteInIsolatedScopeAsync` deleted: zero callers.
...
- `IScopeFactoryTransactionRunner` is `IScopeFactoryUnitOfWorkRunner`: the vocabulary follows the concept the runner serves.
- The container-interface idiom dissolves. Machinery that hid behind an interface only so that its implementation could stay out of sight is plain internal now, below the `_internal`/`_private` sections that superseded the idiom.
