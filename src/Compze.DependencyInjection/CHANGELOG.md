...
- **The async unit-of-work envelope**: `ExecuteUnitOfWorkAsync` (action and result forms) pairs one fresh scope with one transaction whose ambient flows across the function's awaits — one unit of work legitimately migrates across pool threads, its identity its transaction — and `ExecuteInIsolatedScopeAsync` (both forms) is its no-transaction sibling for reads. See `dev_docs/unit-of-work-model.md`, which now records the async-flow-safety contract and the session-affinity consequence: session usage guards assert transaction affinity, never thread affinity.
