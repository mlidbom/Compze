...
partial class MySqlTypeIdInternerPersistence(IMySqlConnectionPool connectionPool, MySqlSqlLayerSchemaManager schemaManager) : ITypeIdInternerPersistence
{
   ...
   // MySQL/InnoDB is MVCC: a suppressed insert commits independently of the business transaction without a
   ...
   public int InsertOrGet(string typeString)
   {
      _connectionPool.UseCommand(command =>
         command.SetCommandText($"INSERT INTO {Schema.TableName} ({Schema.TypeString}) VALUES (@{Schema.TypeString}) ON DUPLICATE KEY UPDATE {Schema.Id} = {Schema.Id}")
