...
namespace Compze.Internals.Sql.MySql.Wiring._internal;

static class MySqlSchemaContributionRegistrar
{
   extension(IComponentRegistrar @this)
   {
      ///<summary>Contributes <paramref name="schemaCreationSql"/> — one feature backend's <see cref="_private.MySqlSchemaContribution"/> —<br/>
      ...
      /// <see cref="MySqlSqlLayerSchemaManager"/> that creates every contributed schema in a single batch before the database's first use.<br/>
      ...
      {
         @this.Register(Singleton.ForSet<_private.MySqlSchemaContribution>()
                                 .CreatedBy(() => new _private.MySqlSchemaContribution(schemaCreationSql)));
      ...
      {
         @this.Register(Singleton.ForSet<_private.MySqlSchemaContribution>()
                                 .CreatedBy((TTableNames tableNames) => new _private.MySqlSchemaContribution(schemaCreationSql(tableNames))));
      ...
      {
         if(!@this.IsRegistered<MySqlSqlLayerSchemaManager>())
         {
            @this.Register(Singleton.For<MySqlSqlLayerSchemaManager>()
                                    .DelegateToParentServiceLocatorWhenCloning()
                                    .CreatedBy((IMySqlConnectionPool connectionPool, IComponentSet<_private.MySqlSchemaContribution> contributions)
                                                  => new MySqlSqlLayerSchemaManager(connectionPool, [..contributions.Select(it => it.SchemaCreationSql)])));
         }
