...
static class ConsoleCE
{
   ...
   static readonly ILock Lock = ILock.WithTimeout(2.Seconds()); //We are just writing to the console. If this blocks for more than a second or so something is very wrong;
   ...
   internal static void WriteLine(string message) => Lock.Update(() => Console.WriteLine(message));
