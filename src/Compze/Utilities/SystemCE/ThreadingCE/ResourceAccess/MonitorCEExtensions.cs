...
static class MonitorCEExtensions
{
   ...
   /// 2. Calls <paramref name="updateOnFailedRead"/> within an update lock if <paramref name="tryRead"/> returns null.
   /// 2.1 Checks once again for the presence of the value after taking the update lock before adding it.
   /// 2.2 Asserts that <paramref name="tryRead"/> returns a value after <paramref name="updateOnFailedRead"/> has been called.
   /// </summary>
