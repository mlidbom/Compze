   ...
   where TEvent : class, ITevent
{
   ...
   IEventHandlerRegistrar<TEvent> ForWrapped<TWrapperEvent>(Action<TWrapperEvent> handler)
      where TWrapperEvent : IWrapperTevent<TEvent>;
   ...
   /// </summary>
   IEventHandlerRegistrar<TEvent> ForGenericEvent<THandledEvent>(Action<THandledEvent> handler) where THandledEvent : ITevent;
...
{
   public static IEventHandlerRegistrar<TEvent> BeforeHandlers<TEvent>(this IEventHandlerRegistrar<TEvent> @this, Action<TEvent> handler) where TEvent : class, ITevent => @this.BeforeHandlers(handler);
   public static IEventHandlerRegistrar<TEvent> AfterHandlers<TEvent>(this IEventHandlerRegistrar<TEvent> @this, Action<TEvent> handler) where TEvent : class, ITevent => @this.AfterHandlers(handler);
}