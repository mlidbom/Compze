   ...
   where TTevent : class, ITevent
{
   ...
   IWrapperTevent<ITevent> EnsureWrapped(TTevent evt)
   {
      if(evt is IWrapperTevent<ITevent> wrapped)
      {
         return wrapped;
      } else
      {
         return WrapperTevent.WrapTevent(evt);
      }
   }
