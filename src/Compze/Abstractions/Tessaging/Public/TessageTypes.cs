...
#pragma warning restore CA1724 //
   ...
   public static class Remotable
   {
      public static class AtMostOnce
      {
         ...
         {
            ///<summary>How and when a <see cref="AtMostOnceTypermediaTommand.Id"/> is generated is vital to correctly maintain the At most once delivery guarantee. When creating the tommand in the backend service we must generate a new <see cref="AtMostOnceTypermediaTommand.Id"/>. When binding the tommand in a UI we must reuse the <see cref="AtMostOnceTypermediaTommand.Id"/> generated by the backend. This enum helps make that important distinction explicit in your code.</summary>
            protected enum DeduplicationIdHandling
            {
               ///<summary>When creating the tommand within the owning handler endpoint a new <see cref="AtMostOnceTypermediaTommand.Id"/> must be generated. </summary>
               Create,
               ///<summary>When binding the tommand in a UI or when deserializing it is very important to NOT create a new <see cref="AtMostOnceTypermediaTommand.Id"/> as this would break the At most once delivery guarantee.</summary>
               Reuse
            }
            ...
            public Guid Id
            {
               ...
               set
               {
                  ...
                  {
                     throw new InvalidOperationException($"Once {nameof(Id)} has been set it cannot be changed. It is only settable at all because many UI technologies require it to be.");
                  }
            ...
            ///<summary>Allows for replacing the <see cref="Id"/> in the rare cases when that is actually helpful. One example is when a tommand failed to execute due to backend business logic rules and you wish to reuse the entered values in the UI that the tommand is bound to. If you do not change the <see cref="Id"/> when doing that the bus will keep returning the response from the first time you sent the tommand.</summary>
