...
#pragma warning restore CA1724 //
   ...
   public static class Remotable
   {
      public static class AtMostOnce
      {
         ...
         {
            ///<summary>How and when a <see cref="AtMostOnceHypermediaTommand.TessageId"/> is generated is vital to correctly maintain the At most once delivery guarantee. When creating the command in the backend service we must generate a new <see cref="AtMostOnceHypermediaTommand.TessageId"/>. When binding the command in a UI we must reuse the <see cref="AtMostOnceHypermediaTommand.TessageId"/> generated by the backend. This enum helps make that important distinction explicit in your code.</summary>
            protected enum DeduplicationIdHandling
            {
               ///<summary>When creating the command within the owning handler endpoint a new <see cref="AtMostOnceHypermediaTommand.TessageId"/> must be generated. </summary>
               Create,
               ///<summary>When binding the command in a UI or when deserializing it is very important to NOT create a new <see cref="AtMostOnceHypermediaTommand.TessageId"/> as this would break the At most once delivery guarantee.</summary>
               Reuse
            }

            ///<summary>How and when a <see cref="TessageId"/> is generated is vital to correctly maintain the At most once delivery guarantee. When creating the command in the backend service we must generate a new <see cref="TessageId"/>. When binding the command in a UI we must reuse the <see cref="TessageId"/> generated by the backend.</summary>
            ...
            ///<summary>Allows for replacing the <see cref="TessageId"/> in the rare cases when that is actually helpful. One example is when a command failed to execute due to backend business logic rules and you wish to reuse the entered values in the UI that the command is bound to. If you do not change the <see cref="TessageId"/> when doing that the bus will keep returning the response from the first time you sent the command.</summary>
