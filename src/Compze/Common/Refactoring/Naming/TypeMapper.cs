...
class TypeMapper : ITypeMapper, ITypeMappingRegistrar
{
   ...
   readonly HashSet<Assembly> _loadedAssemblies = [];
   ...
   public TypeId GetId(Type type) => _state.Update(state =>
   {
      ...
      EnsureAssemblyMappingsLoadedAndContainAllRequiredMappings(type.Assembly);
      return state.TypeToTypeIdMap[type];
   ...
   {
      typesThatRequireMappings.ToList().Select(t => t.Assembly).Distinct().ForEach(EnsureAssemblyMappingsLoadedAndContainAllRequiredMappings);
   ...
   void EnsureAssemblyMappingsLoadedAndContainAllRequiredMappings(Assembly assembly) => _state.Update(state =>
   {
      lock(_loadedAssemblies)
      {
         ...
         if(autoGeneratedType == null) throw BuildExceptionDescribingHowToAddMissingMappings(assembly, state);
         ...
         if(typesWithMissingMappings.Any()) throw BuildExceptionDescribingHowToAddMissingMappings(assembly, state);
   ...
   static Exception BuildExceptionDescribingHowToAddMissingMappings(Assembly assembly, State state)
   {
      var assemblyName = assembly.GetName().Name;
      var rootNamespace = assemblyName;
      var className = "AutoGeneratedCurrentAssemblyTypeMapperForRefactoringSupport";
      var fileName = $"{className}.cs";

      // Get all types that should be mapped in this assembly
      ...
      var existingMappings = allTypesRequiringMapping
      ...
      fixMessage.AppendLine(CultureInfo.InvariantCulture,
                            $"""
                             ...
                             Please create a file named '{fileName}' in the root folder of the project '{assemblyName}' with the following content:

                             """);

      fixMessage.AppendLine(GenerateAutoGeneratedClassCode(rootNamespace, className, allTypesRequiringMapping, existingMappings));

      return new Exception(fixMessage.ToString());
   }

   static string GenerateAutoGeneratedClassCode(string? rootNamespace, string className, ISet<Type> typesToMap, Dictionary<Type, TypeId> existingMappings)
   {
      ...
      code.AppendLine(CultureInfo.InvariantCulture, $"/// </summary>");
      code.AppendLine(CultureInfo.InvariantCulture, $"public sealed class {className} : IMapCurrentAssemblyTypesForRefactoringSupport");
      code.AppendLine("{");
