...
class TypeMapper : ITypeMapper, ITypeMappingRegistrar
{
   ...
   public TypeId GetId(Type type)
   {
      ...
      return _state.Read(state =>
      {
         ...
         throw new Exception($"Type {type.FullName} is not mapped. This usually means it doesn't require mapping (not an IMessage or IHasPersistentIdentity).");
      });
   }
   ...
   public IEnumerable<TypeId> GetIdForTypesAssignableTo(Type type)
   {
      ...
      return _state.Read(state => state
                                   .TypeToTypeIdMap
                                   .Keys
   ...
   void EnsureAllCurrentlyLoadedAssembliesHaveBeenCheckedForRequiredMappings()
   {
      ...
      foreach(var assembly in currentlyLoadedAssemblies)
      {
         lock(_checkedAssemblies)
         {
            ...
            {
               // Ignore assemblies that don't have type mappings or can't be loaded
               // Still add to checked set to avoid repeated failures
               _checkedAssemblies.Add(assembly);
            }
         }
      }
