...
static class OnlyWithinLocksThreadingHelpers
{
   ...
   ///<summary>Must be called from synchronized code that guarantees that this is the only thread modifying <paramref name="original"/>. It's purpose is to allow readers free access to <paramref name="original"/> even though <paramref name="original"/> is only thread safe for concurrent readers.</summary>
   public static void AddRangeToCopyAndReplace<T>(ref IReadOnlyList<T> original, IEnumerable<T> item) =>
      original = original.AddRangeToCopy(item);

   ///<summary>Must be called from synchronized code that guarantees that this is the only thread modifying <paramref name="original"/>. It's purpose is to allow readers free access to <paramref name="original"/> even though <paramref name="original"/> is only thread safe for concurrent readers.</summary>
   public static void AddToCopyAndReplace<T>(ref IReadOnlySet<T> original, T item) =>
      original = original.AddToCopy(item);
   ...
   public static void AddToCopyAndReplace<TKey, TValue>(ref IReadOnlyDictionary<TKey, TValue> original, TKey key, TValue value) where TKey : notnull =>
      original = original.AddToCopy(key, value);

   ///<summary>Must be called from synchronized code that guarantees that this is the only thread modifying <paramref name="original"/>. It's purpose is to allow readers free access to <paramref name="original"/> even though <paramref name="original"/> is only thread safe for concurrent readers.</summary>
   public static void AddRangeToCopyAndReplace<TKey, TValue>(ref IReadOnlyDictionary<TKey, TValue> original, IEnumerable<KeyValuePair<TKey, TValue>> additions) where TKey : notnull =>
