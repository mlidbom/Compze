...
public partial class Outbox : IOutbox
{
   public static void RegisterWith(IComponentRegistrar registrar)
   {
      registrar.Register(Singleton.For<IOutbox>()
                                  .CreatedBy((EndpointConfiguration configuration, IExactlyOnceRoutingClient exactlyOnceRoutingClient, ITessageStorage tessageStorage, IBackgroundExceptionReporter exceptionReporter, OutboxRetryPoller retryPoller)
                                                => new Outbox(exactlyOnceRoutingClient, tessageStorage, configuration, exceptionReporter, retryPoller)));
   ...
   Outbox(IExactlyOnceRoutingClient exactlyOnceRoutingClient, ITessageStorage tessageStorage, EndpointConfiguration configuration, IBackgroundExceptionReporter exceptionReporter, OutboxRetryPoller retryPoller)
   {
   ...
   {
      Contract.State.NotNull(Transaction.Current);
      var connection = _exactlyOnceRoutingClient.ConnectionToHandlerFor(exactlyOnceTommand);
