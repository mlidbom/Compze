...
///<summary>The <see cref="ITransactionIgnoringTeventPublisher"/>: the <see cref="ITeventPublisher"/> with the ambient transaction<br/>
...
[UsedImplicitly] class TransactionIgnoringTeventPublisher : ITransactionIgnoringTeventPublisher
{
   public static void RegisterWith(IComponentRegistrar registrar)
      => registrar.Register(Scoped.For<ITransactionIgnoringTeventPublisher>()
                                  .CreatedBy((ITeventPublisher teventPublisher) => new TransactionIgnoringTeventPublisher(teventPublisher)));

   readonly ITeventPublisher _teventPublisher;

   TransactionIgnoringTeventPublisher(ITeventPublisher teventPublisher) => _teventPublisher = teventPublisher;

   public void Publish(ITevent tevent)
   {
      State.Assert(tevent is not IMustBeSentTransactionally,
                   () => $"{tevent.GetType().FullName} implements {typeof(IMustBeSentTransactionally).FullName}, and immediate, unconditional delivery structurally cannot back a transactional send. Publish it through {typeof(ITeventPublisher).FullName}, which honors the transaction its send must join — the transaction-ignoring publisher is only for tevents whose types demand no transactional send.");
