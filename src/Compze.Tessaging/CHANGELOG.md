...
- Tessaging's transport is protocol-free: one `TessagingRequestHandlers` contribution (`TessagingTransportServer()`) receives arriving tessages and one `TransportMessagePoster` (`TessagingTransportMessagePoster()`) posts them, both over the endpoint transport — the named-pipe and HTTP variants of both are gone, and with them the `Compze.Tessaging.Hosting.AspNetCore` assembly. `NamedPipeTessagingTransport()` now only adds the named-pipe protocol plumbing around them.
