...
> **The `Compze.Typermedia.Client` package folded into `Compze.Tessaging` (the Typermedia sibling of the Tessaging
...
- The typermedia router's reconciliation is signal-driven: between passes it waits on the registry's change signal (`IEndpointRegistry.AwaitPossibleMembershipChange`), so an endpoint announcing or retracting propagates at signal latency instead of at the old once-per-second poll. The periodic pass remains (once a second) for what no signal can carry: a crashed process's addresses disappearing, and retrying connections that failed.
