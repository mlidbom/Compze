...
public static class EndpointRequestExecutor
{
   ...
   public static void ExecuteServerRequestInTransaction(this IEndpoint @this, Action<IServiceBusSession> request) => @this.ServiceLocator.ExecuteTransactionInIsolatedScope(scope => request(scope.Resolve<IServiceBusSession>()));

   //todo: Why would we run a "Server request" without a transaction??
   public static void ExecuteServerRequest(this IEndpoint @this, Action<IServiceBusSession> request) => @this.ServiceLocator.ExecuteInIsolatedScope(scope => request(scope.Resolve<IServiceBusSession>()));
}