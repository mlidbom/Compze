...
public sealed class ExactlyOnceEndpointBuilder : EndpointBuilder
{
   ...
   private protected override void AssertTheFoundationIsDeclared()
   {
      ...
      State.Assert(_registerDomainDatabase is not null,
                   () => "The endpoint declares no domain database. An exactly-once endpoint's durable vertical — inbox, outbox, durable peer memory — lives in the domain database it joins: declare it in the environment's DeclareDomainDatabaseOn, e.g. endpointBuilder.SqliteDomainDatabase(...). An endpoint that deliberately persists nothing is a best-effort endpoint instead (BestEffortEndpointDeclaration).");
   }
