...
/// <see cref="BestEffortEndpointDeclaration{TIdentity}"/>). An endpoint-declaration is what an endpoint IS — its identity,
/// its topology stance, and its handlers — declared once, as a class, so that production and tests host the same endpoint
/// by construction. What varies per deployment — transport, serializer, discovery, the actual
...
/// <see cref="HandlerAvailabilityPatience"/>), and each tessage-handler kind has its own registration override — a virtual method receiving
/// the minimal registrar for exactly that kind — so a declaration overrides only the registrations its endpoint actually serves.
/// Wiring the registration overrides do not cover — store integrations, arbitrary component registration — goes through the tier's
/// general <c>Declare</c> override, which receives the full <see cref="EndpointBuilder"/> (the tier's subtype).
///</summary>
...
public abstract class EndpointDeclaration<TIdentity> where TIdentity : IEndpointIdentity
{
   ...
   ///<summary>Override to register tevent subscriptions that do not demand exactly-once delivery — see <see cref="IBestEffortTeventHandlerRegistrar"/>.</summary>
   protected virtual void RegisterBestEffortTeventHandlers(IBestEffortTeventHandlerRegistrar handle) {}

   ///<summary>Override to register handlers for the navigated tommand kinds — see <see cref="ITypermediaTommandHandlerRegistrar"/>.</summary>
   protected virtual void RegisterTypermediaTommandHandlers(ITypermediaTommandHandlerRegistrar handle) {}

   ///<summary>Override to register tuery handlers — see <see cref="ITueryHandlerRegistrar"/>.</summary>
   protected virtual void RegisterTueryHandlers(ITueryHandlerRegistrar handle) {}

   ///<summary>Override to register tevent observers — observation, the deliberately transaction-ignoring watch surface; see <see cref="ITeventObservationRegistrar"/>.</summary>
   protected virtual void ObserveTevents(ITeventObservationRegistrar observe) {}

   ///<summary>Declares everything the tiers share onto the builder: the scalar aspects and the shared registration overrides. The tier's own<br/>
   /// <c>Build</c> is the template — environment first, then this, then the tier's own registration overrides, then the general<br/>
   /// <c>Declare</c> override, then the build.</summary>
   private protected void ConfigureSharedAspects(EndpointBuilder builder)
   {
