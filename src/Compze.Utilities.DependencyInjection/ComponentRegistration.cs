...
public abstract class ComponentRegistration
{
                                  ...
                                  IEnumerable<Type> dependencyTypes)
   {
      ...
      ContractAssertion.Argument.Fulfills(
...
public class ComponentRegistration<TService> : ComponentRegistration where TService : class
{
   ...
   {
      ContractAssertion.State.Fulfills(
   ...
   public override ComponentRegistration CreateCloneRegistration(IServiceLocator currentLocator)
   {
      ...
      ContractAssertion.State.Fulfills(Lifestyle == Lifestyle.Singleton, () => "Only Singletons can delegate to parent container when cloning, because otherwise both containers would attempt to dispose the component");
