...
partial class SqliteTypeIdInternerPersistence(ISqliteConnectionPool connectionPool, SqliteSqlLayerSchemaManager schemaManager) : ITypeIdInternerPersistence
{
   ...
   // SQLite is single-writer: a suppressed insert would deadlock against the business transaction's writer lock,
   // so interning joins the ambient transaction instead.
   public bool SuppressAmbientTransactionBeforeAllCalls => false;
   ...
   {
      _connectionPool.UseCommand(command => command.SetCommandText($"INSERT OR IGNORE INTO {Schema.TableName} ({Schema.TypeString}) VALUES (@{Schema.TypeString})")
                                                   .AddMediumTextParameter(Schema.TypeString, typeString)
                                                   .ExecuteNonQuery());
