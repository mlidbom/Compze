...
public ref struct ErrorLogInterpolatedStringHandler
{
   ...
   public ErrorLogInterpolatedStringHandler(int literalLength, int formattedCount, ILogger logger, out bool enabled)
   {
      enabled = !CompzeLogger.LoggingSuppressed && logger.IsEnabled(LogLevel.Error);
