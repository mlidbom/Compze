...
public ref struct CriticalLogInterpolatedStringHandler
{
   ...
   public CriticalLogInterpolatedStringHandler(int literalLength, int formattedCount, ILogger logger, out bool enabled)
   {
      enabled = !CompzeLogger.LoggingSuppressed && logger.IsEnabled(LogLevel.Critical);
