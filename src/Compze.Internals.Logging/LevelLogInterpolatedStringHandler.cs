...
public ref struct LevelLogInterpolatedStringHandler
{
   LogInterpolatedStringHandlerCore _core;

   public LevelLogInterpolatedStringHandler(int literalLength, int formattedCount, ILevelLogger logger, out bool enabled)
   {
      enabled = !CompzeLogger.LoggingSuppressed && logger.IsEnabled();
