...
public interface IShared
{
   ...
   public static IShared<TShared> New<TShared>(TShared shared, ICriticalSection @lock) =>
      new Shared<TShared>(shared, @lock);

   internal class Shared<TShared>(TShared shared, ICriticalSection @lock) : IShared<TShared>
   {
      readonly TShared _shared = shared;
      public ICriticalSection Lock { get; } = @lock;
