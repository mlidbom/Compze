...
partial class MySqlEndpointCatalogSqlLayer(IMySqlConnectionPool connectionFactory, MySqlSqlLayerSchemaManager schemaManager) : ITessagingSqlLayer.IEndpointCatalogSqlLayer
{
      ...
      try
      {
         ...
         await using(command.caf())
         {
            ...
            if(lockResult is 1L)
            {
               ...
               var session = new ProcessLockSession(connection, lockSession => ReleaseGetLockOnTheLockSessionAsync(lockSession, endpointName), onLockLostWhileHeld);
   ...
   //Releases the session-scoped GET_LOCK explicitly, so disposal returns only once the server has released it rather than
   //whenever it gets round to noticing the connection close. The name is recomputed from the same database and endpoint name.
   static async Task ReleaseGetLockOnTheLockSessionAsync(DbConnection lockSession, string endpointName)
   {
      var command = lockSession.CreateCommand();
      await using(command.caf())
      {
         ...
         parameter.Value = ProcessLockKeys.Name(lockSession.Database, endpointName);
