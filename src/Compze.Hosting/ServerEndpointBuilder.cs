...
class ServerEndpointBuilder : IEndpointBuilder
{
   ...
   public ITypeIdentifierMapper TypeMapper { get; }
...
TypeMapper = new TypeIdentifierMapper();
   ...
   void SetupContainer()
   {
((TypeIdentifierMapper)TypeMapper).MapTypesFromAllLoadedAssembliesWithTypeMappingsAttribute();

      var register = Registrar;
      //Universal stuff here
      register.JSonAppConfigFileConfigurationParameterProvider();
      register.Register(Singleton.For<ITypeIdentifierMapper>().Instance(TypeMapper));
