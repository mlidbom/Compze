...
///<summary>Identifies the OS process that published an endpoint address into an <see cref="InterprocessEndpointRegistry"/>:<br/>
...
/// The registry uses it to recognize addresses whose publishing process has exited, so a crashed process's stale addresses are never routed to.</summary>
public class PublishingProcess
{
   ///<summary>The process this code runs in — what a process publishing its own endpoints' addresses passes.</summary>
   public static PublishingProcess Current
   {
      get
      {
         using var currentProcess = Process.GetCurrentProcess();
         return new PublishingProcess(currentProcess.Id, currentProcess.StartTime.ToUniversalTime().Ticks);
      }
   }
   ...
   public PublishingProcess(int processId, long startTimeTicks)
   {
      ProcessId = processId;
